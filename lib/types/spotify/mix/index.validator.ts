...
export const isSpotifyMixType = ajv.compile(
  SpotifyMixTypeSchema,
) as ValidateFunction<SpotifyMixType>;
export default function validateSpotifyMix(value: unknown): SpotifyMixType {
  if (isSpotifyMixType(value)) {
    return value;
  } else {
    throw new Error(
      ajv.errorsText(
        isSpotifyMixType.errors!.filter((e: any) => e.keyword !== 'if'),
        {dataVar: 'SpotifyMixType'},
      ) +
        '\n\n' +
        inspect(value),
    );
