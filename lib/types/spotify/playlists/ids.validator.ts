...
import Ajv from 'ajv';
import SpotifyPlaylistsType from './ids';
...
export const SpotifyPlaylistsTypeSchema = {
...
export const isSpotifyPlaylistsType = ajv.compile(
  SpotifyPlaylistsTypeSchema,
) as ValidateFunction<SpotifyPlaylistsType>;
export default function validateSpotifyPlaylists(
  value: unknown,
): SpotifyPlaylistsType {
  if (isSpotifyPlaylistsType(value)) {
    return value;
  } else {
    throw new Error(
      ajv.errorsText(
        isSpotifyPlaylistsType.errors!.filter((e: any) => e.keyword !== 'if'),
        {dataVar: 'SpotifyPlaylistsType'},
      ) +
        '\n\n' +
        inspect(value),
    );
